   ...
   io.use((socket, next) => {
      ...
      if (socket.request.session?.passport?.user) {
         ...
         passport.deserializeUser(socket.request.session.passport.user, (err, user) => {
            if (err) return next(err) // 에러발생시 에러미들웨어로 전송
            socket.request.user = user // 역직렬화 된 사용자 정보를 socket request 객체에 저장
            next() // 다음 미들웨워 진행
         })
      } else {
         // 로그인이 안된 상태에서 소켓에 접속시 소켓 연결 해제(로그인 한 사람만 채팅 가능하도록 함)
